...
```java
    ...
    map.getOrDefault(K key, V defaultValue) // 没找到就返回设置的默认值
    map.remove(K key);
...
```java
    ...
    Collections.reverse(List list); // 翻转list，不能是数组
...
2. 去重
    ...
    固定第一个数，但是因为第二个数可以和第一个数相同，所以用nums[i]和nums[i-1]比较
